...
class Data:
    ...
    def pre_processing_fixes(self, ratings):
        # Fix nationality
        self.export.rename(columns={"Nat": "1st Nat", "Nat.1": "Nat"})

        # Calculate feet scores
        self.export["Lft"] = self.export["Left Foot"].map(ratings["feet"])
        self.export["Rft"] = self.export["Right Foot"].map(ratings["feet"])
        self.export["Feet"] = self.export["Lft"] + self.export["Rft"]

    def post_processing_fixes(self, ratings):
        output = self.export[
            [col for col in self.config["output_columns"] if col in self.export.columns]
        ]

        # Append the calculated columns to the output
        for key, data in ratings[self.config["ratings_set"]].items():
    ...
    def calculator(
        ...
        positions = ratings_data["positions"]
        ...
        df.loc[eligible_players, ratings_key.upper()] = (
            df.loc[eligible_players, att_weightings.keys()].sum(axis=1)
            / theoretical_max_score
            * 100
        )

        return df[ratings_key.upper()].round(2).astype(float)
